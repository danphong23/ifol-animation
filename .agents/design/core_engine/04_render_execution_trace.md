...
[
  ...
  RootGraph_Viewport_1: [
      ...
      DrawCommand (10.000 lá cỏ - ĐÃ ĐƯỢC GỘP), 
      DrawCommand (Khung hình Video),
      DrawCommand (Viên đạn - bị thiếu ảnh)
  ],
  // Đích đến: Cửa sổ Viewport 2
  RootGraph_Viewport_2: [
      DrawCommand (Chỉ vẽ lại kết quả của SubGraph_Character)
  ]
]
```
...
*   Nó đi xin VRAM một tấm ảnh trống (Render Target).
*   Nó đi sâu vào trong SubGraph_Character, thực thi các Draw Command bên trong đó (vẽ tay, chân, mặt) và in tất cả lên tấm ảnh trống vừa xin.
*   Vẽ xong, nó cất tấm ảnh đó vào Cache với tên `"cache_char_01"`. Quay lại mạch vẽ chính.

### Hành động 3.2: Gặp DrawCommand (10.000 chiếc lá cỏ)
...
*   Engine gọi `pipeline_id = "instanced_shape"`, nạp ảnh `grass.png`.
*   GPU giật điện, vẽ 10.000 chiếc lá trong 1 nhịp chớp mắt. Cực mượt.

### Hành động 3.3: Gặp DrawCommand (Khung hình Video)
*   Engine đọc lệnh thấy yêu cầu dùng `Texture_ID = "video_frame"`.
...
### Hành động 3.4: Gặp DrawCommand (Viên đạn)
*   Engine đọc lệnh yêu cầu nạp ảnh `bullet.png`.
*   **Xử lý Edge Case (Missing File):** Không tìm thấy file trên ổ cứng. Nếu là engine nghiệp dư, tới đây sẽ văng màn hình xanh lỗi `NullReference`.
*   Engine của chúng ta bình tĩnh lôi tấm ảnh `"checkerboard.png"` (ảnh caro hồng/đen mờ) có sẵn trong lõi ra thế chỗ.
*   Lệnh vẽ tiếp tục, viên đạn biến thành hình vuông caro lơ lửng trên màn hình (Báo hiệu cho user biết lỗi thiếu file, nhưng phần mềm vẫn sống nhăn răng).
...
*   ECS đã tính toán sẵn sự tối ưu này. Lệnh vẽ nó gửi xuống chỉ là: `DrawCommand(Pipeline: "image", Texture: "cache_char_01")`.
*   Engine thấy lệnh này, nó lôi tấm ảnh `"cache_char_01"` (mà nó vừa cực khổ vẽ ở hành động 3.1) ra, phóng to lên và in thẳng ra Viewport 2.
*   **Kết quả:** Dù nhân vật có 100 cái xương, GPU không tốn thêm 1 giọt mồ hôi nào để tính toán lại. Nó chỉ thực hiện 1 lệnh copy ảnh rẻ bèo.
