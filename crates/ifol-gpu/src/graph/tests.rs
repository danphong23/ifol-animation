...
fn flatten_orders_child_nodes_before_subgraph_composite() {
    ...
    assert_eq!(
        plan.nodes
            ...
            .collect::<Vec<_>>(),
        vec![child_batch, subgraph, root_batch]
    );
...
fn flatten_keeps_extension_node_and_uses_its_resource_hazards() {
    ...
    graph.declare_resource_usage(
        reader,
        GraphResource::Texture(TextureHandle(9)),
        ResourceAccess::Read,
    );
...
fn resource_write_then_read_creates_implicit_hazard_edge() {
    ...
    graph.declare_resource_usage(
        writer,
        GraphResource::Buffer(BufferHandle(1)),
...
fn copy_commands_infer_source_read_and_destination_write_hazard() {
    let mut pool = RenderNodePool::new();
    let copy = pool.alloc_copy_batch(vec![CopyCommand::buffer_to_buffer(
        BufferHandle(1),
        BufferHandle(2),
        4,
    )]);
    ...
    assert_eq!(
        graph.ordered_node_ids(&pool).unwrap(),
        vec![copy, later_read]
    );
}
...
fn texture_copy_hazard_uses_mip_and_layer_range() {
    ...
    let copy = pool.alloc_copy_batch(vec![CopyCommand::TextureToTexture {
        source: TextureHandle(1),
        destination: TextureHandle(2),
        source_mip_level: 0,
    ...
    graph.declare_texture_subresource_usage(
        later_writer,
        TextureHandle(1),
        1,
        0,
...
fn buffer_copy_hazard_uses_byte_range() {
    ...
    let copy = pool.alloc_copy_batch(vec![CopyCommand::BufferToBuffer {
        source: BufferHandle(1),
        destination: BufferHandle(2),
        source_offset: 0,
...
fn disjoint_depth_and_stencil_aspects_do_not_create_hazard_edge() {
    ...
    graph.declare_texture_aspect_usage(
        depth_writer,
        TextureHandle(9),
        0,
        0,
    ...
    assert_eq!(
        graph.ordered_node_ids(&pool).unwrap(),
        vec![stencil_writer, depth_writer]
    );
}

#[test]
fn all_texture_aspect_overlaps_depth_and_stencil() {
    assert!(aspects_overlap(
        TextureAspect::All,
        TextureAspect::DepthOnly
    ));
    assert!(aspects_overlap(
...
fn flatten_applies_hazard_between_nested_and_root_nodes() {
    ...
    child.declare_resource_usage(
        nested_writer,
        GraphResource::Texture(TextureHandle(10)),
        ResourceAccess::Write,
    );
    ...
    assert_eq!(
        plan.nodes
            ...
            .map(|node| node.node_id)
            .collect::<Vec<_>>(),
        vec![nested_writer, subgraph, reader]
    );
}
