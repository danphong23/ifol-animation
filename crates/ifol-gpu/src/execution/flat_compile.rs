...
pub(crate) fn map_graph_flatten_error(error: GraphFlattenError) -> RenderGraphValidationError {
    match error {
        GraphFlattenError::MissingNode(node) => RenderGraphValidationError::MissingNode(node),
        GraphFlattenError::Cycle(node) => RenderGraphValidationError::DependencyCycle(node),
        GraphFlattenError::DependencyNodeOutsideGraph(node) => {
