...
fn main() {
    ...
    // Execute
    let _ = executor.execute_checked(&engine, &registry, &mut pool, &graph_z).expect("visual graph must validate");
    let _ = executor.execute_checked(&engine, &registry, &mut pool, &graph_alpha).expect("visual graph must validate");
    let _ = executor.execute_checked(&engine, &registry, &mut pool, &graph_10k).expect("visual graph must validate");
    let idx_last = executor.execute_checked(&engine, &registry, &mut pool, &graph_interleaved).expect("visual graph must validate");
