...
fn run_tc34_directional_blur() {
    ...
    pollster::block_on(async {
        ...
        let pipe_blur = h.register_pipeline("directional_blur.wgsl", Some(wgpu::BlendState::ALPHA_BLENDING), false, true);
        ...
        let blur_uni = DirBlurUniform {
            angle: std::f32::consts::PI / 6.0, // 30 degrees
            strength: 0.05,
            samples: 20.0,
            _pad0: 0.0,
        };
        let bg_blur_uni = h.create_custom_uniform_bind_group(blur_uni, "DirBlur Uniform");
        ...
        // Pass 2: Apply Directional Blur over dark background
        let mut graph_final = RenderGraph::new(RenderTarget::Offscreen {
            color: final_target_id,
            width: 800,
            height: 600,
        }).with_clear_color([0.2, 0.1, 0.1, 1.0]);
        ...
        let graph_json = serde_json::json!({
            "test_case": "TC34 - Directional Motion Blur",
            "features": [
                "Vector-based Sampling Loop",
                "Speed simulation blur"
            ]
        });
        fs::create_dir_all("tests/graphs").unwrap();
        fs::write("tests/graphs/tc34_directional_blur.json", serde_json::to_string_pretty(&graph_json).unwrap()).unwrap();

        h.execute_and_record(
            &graph_final,
            &final_target_tex,
            "tc34_directional_blur",
            "Directional Motion Blur",
            "Làm nhòe hình ảnh theo hướng 30 độ. Sử dụng cho các chuyển cảnh trượt ngang (Slide Wipe) hoặc giả lập tốc độ cao (Speed Blur).",
            "Kiểm tra vòng lặp lấy mẫu (Sampling loop) và hiệu suất bộ đệm mờ (Accumulation buffer).",
        );
    });
}
