...
#[test]
fn test_tc80_msdf_text() {
    pollster::block_on(async {
        let mut h = harness::DesktopTestHarness::new(800, 600).await;
        let (target_handle, target_tex) = h.create_target("tc80_target");

        // Use pattern.png as our dummy MSDF texture.
        // In a real scenario, this would be an MSDF font atlas generated by msdfgen.
        let msdf_tex_info = h.load_texture("mask_star.jpeg");

        let bg_h = h.create_texture_bind_group(msdf_tex_info.handle, "msdf_bg");

        let shader_code = std::fs::read_to_string("tests/shared_assets/shaders/render_msdf.wgsl").unwrap();
        let shader = h.engine.device().create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("render_msdf.wgsl"),
            source: wgpu::ShaderSource::Wgsl(std::borrow::Cow::Borrowed(&shader_code)),
        });

        let layout = h.engine.device().create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("msdf_layout"), bind_group_layouts: &[Some(&h.texture_bg_layout)], immediate_size: 0,
        });

        let pipe = h.engine.device().create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("render_msdf"), layout: Some(&layout),
        ...
        println!("MSDF Text Render Time: {:?}", t_elapsed);

        h.execute_and_record(&graph, &target_tex, "tc80_msdf_text", "MSDF Text Rendering", "Evaluating Multi-channel Signed Distance Field texture for crisp vector rendering with outline", "Render output");
    });
}
