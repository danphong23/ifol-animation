...
use ifol_gpu::resources::{
    ...
    ComputePipelineHandle, MeshHandle, PipelineHandle, PipelineLayoutResourceDescriptor,
...
impl<'a> DesktopTestHarness<'a> {
    ...
    pub fn create_dual_texture_bind_group(
        &mut self,
        t_handle_a: TextureHandle,
        t_handle_b: TextureHandle,
        label: &str,
    ) -> BindGroupHandle {
    ...
    pub fn create_custom_uniform_bind_group<T: bytemuck::Pod>(
        &mut self,
        uniform: T,
        label: &str,
    ) -> BindGroupHandle {
    ...
    pub fn register_pipeline(
        ...
        let mut bind_group_layouts: Vec<Option<&wgpu::BindGroupLayout>> =
            vec![Some(&self.texture_bg_layout)];
        if has_uniform {
        ...
        self.registry.insert_pipeline_with_layout_descriptor(
            ...
            PipelineLayoutResourceDescriptor {
                bind_group_layout_signatures: if has_uniform {
                    vec![Some(1), Some(2)]
                } else {
                    vec![Some(1)]
                },
    ...
    pub fn register_sky_pipeline(&mut self) -> PipelineHandle {
        ...
        let layout = self
            ...
            .create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                ...
                bind_group_layouts: &[Some(&self.texture_bg_layout), Some(&self.uniform_bg_layout)],
                immediate_size: 0,
            });
    ...
    pub fn register_splitscreen_pipeline(&mut self) -> PipelineHandle {
        ...
        let layout = self
            ...
            .create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                ...
                bind_group_layouts: &[Some(&self.texture_bg_layout), Some(&self.texture_bg_layout)],
                immediate_size: 0,
            });
    ...
    pub fn execute_and_record(
        ...
        let sub_idx_1 = self
            .executor
            .execute_checked(&self.engine, &self.registry, &mut self.pool, graph)
        ...
        let actual_rendered_tex = match graph.target {
            RenderTarget::Offscreen { color, .. } => {
                self.registry.owned_texture(&color).unwrap_or(target_tex)
            }
            RenderTarget::OffscreenMsaa { resolve, .. } => {
                self.registry.owned_texture(&resolve).unwrap_or(target_tex)
            }
            _ => target_tex,
        };
    ...
    pub fn register_stencil_pipeline(
        &mut self,
        shader_filename: &str,
    ...
    pub fn register_compute_pipeline(
        ...
        let bgl_options: Vec<Option<&wgpu::BindGroupLayout>> =
        ...
        self.registry
            .insert_compute_pipeline_with_layout_descriptor(
                ...
                pipeline,
                ifol_gpu::resources::PipelineLayoutResourceDescriptor {
                    bind_group_layout_signatures: (0..bind_group_layouts.len())
    ...
    pub fn readback_storage_buffer<T: bytemuck::Pod + Clone>(
        &self,
        buffer: &wgpu::Buffer,
        count: usize,
    ) -> Vec<T> {
