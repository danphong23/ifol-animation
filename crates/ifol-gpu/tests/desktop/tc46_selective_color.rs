...
fn run_tc46_selective_color() {
    ...
    pollster::block_on(async {
        ...
        let pipe_selective = h.register_pipeline("selective_color.wgsl", Some(wgpu::BlendState::ALPHA_BLENDING), false, true);

        // Paladin (Pink/Red Cape and Armor)
        ...
        // Target Hue = 0.95 (Pink/Red of Paladin armor/cape), desaturate all blue Sci-Fi background to grayscale
        let select_uni = SelectiveColorUniform {
            target_hue: 0.95,
            tolerance: 0.08,
            softness: 0.05,
            saturation_boost: 1.4,
        };
        let bg_select_uni = h.create_custom_uniform_bind_group(select_uni, "SelectiveColor Uniform");
        ...
        // Pass 2: Apply Selective Color Isolation (Sin City Grayscale + Red/Pink Pop)
        ...
        graph_final.add_batch(
            &mut h.pool,
            vec![
                DrawCommand::new(pipe_selective, DrawAction::Procedural { vertex_count: 6, instance_range: 0..1 })
                    .with_bind_group(0, bg_tex_scene, Vec::new())
                    .with_bind_group(1, bg_select_uni, Vec::new()),
            ],
        );
        ...
        let graph_json = serde_json::json!({
            "test_case": "TC46 - Selective Color Isolation & Hue Pass",
            "features": [
                "RGB to HSV Color Space Conversion",
                "Circular Hue Distance Analysis",
                "Selective Desaturation (Sin City Cinematic Effect)"
            ]
        });
        fs::create_dir_all("tests/graphs").unwrap();
        fs::write("tests/graphs/tc46_selective_color.json", serde_json::to_string_pretty(&graph_json).unwrap()).unwrap();

        h.execute_and_record(
            &graph_final,
            &final_target_tex,
            "tc46_selective_color",
            "Selective Color Isolation",
            "Hiệu ứng tách màu điện ảnh (Sin City Effect): Chuyển toàn bộ khung cảnh về đen trắng (Grayscale), chỉ giữ lại màu đỏ/hồng của giáp Paladin với độ bão hòa cao và biên chuyển tiếp màu mượt mà.",
            "Xác thực thuật toán phân tích góc màu hình tròn trên không gian màu HSV và khử răng cưa dải màu.",
        );
    });
}
