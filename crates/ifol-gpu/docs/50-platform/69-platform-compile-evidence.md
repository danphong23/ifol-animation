...
| Host Vulkan probe | `cargo test -p ifol-gpu backend::builder::tests --lib -- --test-threads=1` | Đạt | builder policy reaches Vulkan adapter request and exposes selected adapter identity |
| Host GL probe | `cargo test -p ifol-gpu backend::builder::tests --lib -- --test-threads=1` | Đạt | GL adapter request succeeds hoặc trả optional no-adapter outcome |
| Host DX12 probe | `cargo test -p ifol-gpu backend::builder::tests --lib -- --test-threads=1` | Đạt | DX12 adapter request succeeds hoặc trả optional no-adapter outcome |
| Host fallback probe | `cargo test -p ifol-gpu backend::builder::tests --lib -- --test-threads=1` | Đạt | fallback policy reaches adapter request; availability remains adapter-dependent |
