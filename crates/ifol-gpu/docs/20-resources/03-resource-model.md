...
Các map nội bộ của `ResourceRegistry` đã được private hóa. Mutation đi qua các
API `insert_*_with_descriptor`, owned-resource và `remove_*`, cùng version
tracking để compiled artifact được invalidate an toàn.
